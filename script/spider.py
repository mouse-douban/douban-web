...
def get_subjects(tag, limit, start):
    ...
    print(f"request {url}")
    resp = requests.get(url=url, headers={"Referer": "from Gallifrey",
...
def get_subject_info(mid):
    ...
    for pl in subject_detail.find_all("span", class_="pl"):
        if not ["制片国家/地区:", "语言:", "又名:", "IMDb:"].__contains__(pl.text):
            continue
    ...
    detail["片长"] = subject_detail.find("span", property="v:runtime").text.strip("分钟")
